...
def merge_charts_features(charts, features_by_id, features_by_text, features_by_first_artist, audio_cols):
    ...
    # Maak een tijdelijke in-memory database aan
    conn = sqlite3.connect(":memory:")
    
    # Laad de charts en de verschillende feature-lookups in SQL
    charts.to_sql("charts_table", conn, index=False)
    ...
    SELECT 
        c.*, 
        COALESCE(f1.valence, f2.valence) as valence,
        COALESCE(f1.energy, f2.energy) as energy,
        COALESCE(f1.danceability, f2.danceability) as danceability,
        COALESCE(f1.acousticness, f2.acousticness) as acousticness,
        COALESCE(f1.track_genre, f2.track_genre) as track_genre,
        CASE 
            WHEN f1.valence IS NOT NULL THEN 'track_id'
            WHEN f2.valence IS NOT NULL THEN 'title_artist'
            ELSE NULL 
        END as merge_method
    FROM charts_table c
    LEFT JOIN feat_id f1 ON c.track_id = f1.track_id
    LEFT JOIN feat_text f2 ON (c.title_clean = f2.title_clean AND c.artist_clean = f2.artist_clean)
    """
    
    song_level = pd.read_sql_query(query, conn)
    conn.close()

    # Zet de numerieke kolommen goed (SQL maakt soms objecten van NaNs)
    for col in audio_cols:
        if col in song_level.columns:
    ...
    print("Mergecontrole voltooid:")
    print(f"- Totaal rijen: {total_rows}")
    print(f"- Gematched met valence: {matched_rows} ({round(match_rate, 2)}%)")
    print("- Matchmethodes gevonden via SQL:")
