...
#
# DEPRESSIEVE SONG DEFINITIE:
# - sad_song = valence <= SAD_THRESHOLD EN energy <= LOW_ENERGY_THRESHOLD
# - combinatie van laag geluk (valence) en lage energie
# ============================================================
...
SAD_THRESHOLD = 0.40
LOW_ENERGY_THRESHOLD = 0.50   # Pas dit aan indien gewenst (0.0 - 1.0)
...
if "energy" in features_small.columns:
    print("Rijen met energy:", features_small["energy"].notna().sum())
...
# ============================================================
# STAP 5: MERGEN MET VALENCE + ENERGY
# ============================================================
...
# DEPRESSIEVE SONG DEFINITIE:
# - sad_song = valence <= SAD_THRESHOLD EN energy <= LOW_ENERGY_THRESHOLD
# - Als energy niet beschikbaar is voor een track, telt alleen valence
# ============================================================

usable = song_level.dropna(subset=["date", "valence"]).copy()

# Bepaal energy-beschikbaarheid op songniveau
has_energy = "energy" in usable.columns and usable["energy"].notna().any()

if has_energy:
    print(f"\nDepressieve song definitie: valence <= {SAD_THRESHOLD} EN energy <= {LOW_ENERGY_THRESHOLD}")
    print("(Als energy ontbreekt voor een track: alleen valence-filter)")

    # Primaire definitie: beide filters
    both_available = usable["energy"].notna()
    usable.loc[both_available, "sad_song"] = (
        (usable.loc[both_available, "valence"] <= SAD_THRESHOLD) &
        (usable.loc[both_available, "energy"] <= LOW_ENERGY_THRESHOLD)
    )
    # Fallback: alleen valence als energy ontbreekt
    usable.loc[~both_available, "sad_song"] = (
        usable.loc[~both_available, "valence"] <= SAD_THRESHOLD
    )
else:
    print(f"\nGeen energy-kolom gevonden. Depressieve song definitie: valence <= {SAD_THRESHOLD}")
    usable["sad_song"] = usable["valence"] <= SAD_THRESHOLD

usable["sad_song"] = usable["sad_song"].astype(bool)

# Coverage berekenen op basis van tracks waarvoor we een oordeel kunnen geven
# (i.e. tracks met valence; energy-filter is een extra eis bovenop valence)
...
# Gemiddelde energy per dag (informatief)
if has_energy:
    energy_usable = usable.dropna(subset=["energy"]).copy()
    daily_energy = energy_usable.groupby("date").agg(
        avg_energy=("energy", "mean"),
        tracks_with_energy=("energy", "count")
    ).reset_index()
    daily_metrics = pd.merge(daily_metrics, daily_energy, on="date", how="left")
...
if "streams" in song_level.columns:
    stream_usable = usable.dropna(subset=["streams"]).copy()
    ...
    if not stream_usable.empty:
        ...
        for date_value, group in stream_usable.groupby("date"):
            ...
            if total_streams > 0:
                ...
                sad_streams = group.loc[group["sad_song"], "streams"].sum()
...
print("\nGebruikte depressieve-song definitie:")
if has_energy:
    print(f"sad_song = valence <= {SAD_THRESHOLD} EN energy <= {LOW_ENERGY_THRESHOLD}")
else:
    print(f"sad_song = valence <= {SAD_THRESHOLD} (geen energy beschikbaar)")
