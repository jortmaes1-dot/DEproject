# ============================================================
# FINAL_ANALYSIS.PY - RESEARCH PIPELINE V3
# ============================================================
# DOEL:
...
#
# HOOFDONDERZOEKEN
# 1. Gemiddelde valence van de Top 200 op regendagen vs droge dagen.
# 2. Aandeel sad songs / sad streams op regendagen vs droge dagen.
#
# EXTRA ONDERZOEKEN
...
# Belangrijke keuzes:
# - Geen scatterplots.
# - Geen neutrale/mixed weercategorie in de finale vergelijkingen.
# - Regenachtig = prcp > 0.
# - Droog = prcp == 0.
...
def add_weather_variables(df):
    """Maakt weerkolommen zonder neutrale categorie voor de finale analyse."""
    df = df.copy()
    ...
    df["rain_dry_type"] = np.where(df["rainy_day"], RAINY_LABEL, DRY_LABEL)

    # Scores blijven nuttig voor PowerBI, maar statistisch testen we prcp en tsun rechtstreeks.
    df["weather_score_sunny"] = zscore(df["tsun"]) - zscore(df["prcp"])
    df["weather_score_rainy"] = zscore(df["prcp"]) - zscore(df["tsun"])
...
def welch_and_mannwhitney(data, group_col, value_col, group_a=RAINY_LABEL, group_b=DRY_LABEL):
    result = {
        "group_a": group_a,
        "group_b": group_b,
        "value": value_col,
...
def load_song_weather(daily):
    ...
    weather_cols = [
        "date", "prcp", "tsun", "rainy_day", "dry_day", "rain_dry_type",
        "season", "year", "month",
    ]
...
def analyse_main_weather(daily, significance_rows):
    ...
    pbi_cols = [
        "date", "year", "month", "season", "prcp", "tsun", "rainy_day", "dry_day", "rain_dry_type",
    ...
    # Regenachtig vs droog voor gemiddelde valence en sad share.
    t_rows = []
    for metric, metric_label in metric_info.items():
        res = welch_and_mannwhitney(daily, "rain_dry_type", metric, RAINY_LABEL, DRY_LABEL)
        res["metric"] = metric
        ...
        t_rows.append(res)
        add_significance_row(significance_rows, analysis_name, "Welch t-test", metric_label, f"{RAINY_LABEL} vs {DRY_LABEL}", res["welch_p"], res["welch_t"], res["n_a"] + res["n_b"])
        add_significance_row(significance_rows, analysis_name, "Mann-Whitney U", metric_label, f"{RAINY_LABEL} vs {DRY_LABEL}", res["mannwhitney_p"], res["mannwhitney_u"], res["n_a"] + res["n_b"])

    t_df = pd.DataFrame(t_rows)
    save_csv_dual(t_df, stats_filename="stats_01_main_rain_vs_dry_tests.csv", powerbi_filename="pbi_01_main_rain_vs_dry_tests.csv")

    # PowerBI bar-data: gemiddelden per weertype.
    bar_rows = []
    for metric, metric_label in metric_info.items():
        tmp = daily.groupby("rain_dry_type", as_index=False).agg(
            n_days=("date", "count"),
    ...
    save_csv_dual(bar_df, table_filename="table_01_rain_vs_dry_summary.csv", powerbi_filename="pbi_01_rain_vs_dry_summary.csv")
    ...
    for ax, (metric, metric_label) in zip(axes, metric_info.items()):
        ...
        plot_df["order"] = plot_df["rain_dry_type"].map({RAINY_LABEL: 1, DRY_LABEL: 2})
        plot_df = plot_df.sort_values("order")
        ax.bar(plot_df["rain_dry_type"], plot_df["mean_value"])
        ax.set_title(metric_label)
        ax.set_xlabel("Weertype")
        ax.set_ylabel("Gemiddelde waarde")
        ax.tick_params(axis="x", rotation=20)
    fig.suptitle("Hoofdonderzoek: regendagen vs droge dagen", fontsize=14, fontweight="bold")
    fig.tight_layout(rect=[0, 0, 1, 0.93])
    fig.savefig(FIG_DIR / "fig_01_main_rain_vs_dry_bars.png", dpi=200)
    plt.close(fig)
...
def create_entity_daily_shares(song_weather, entity_col):
    ...
    weather_cols = ["date", "prcp", "tsun", "rain_dry_type", "season", "year", "month"]
...
def analyse_audiofeatures_weather(daily, significance_rows):
    ...
    for feature, col in feature_map.items():
        for _, row in daily[["date", "prcp", "tsun", "rain_dry_type", col]].dropna(subset=[col]).iterrows():
            long_rows.append({
                ...
                "prcp": row["prcp"],
                "tsun": row["tsun"],
                "rain_dry_type": row["rain_dry_type"],
            })
    ...
    # Regenachtig vs droog voor audiofeatures als extra, maar nog steeds direct weer-gerelateerd.
    t_rows = []
    for feature, col in feature_map.items():
        res = welch_and_mannwhitney(daily, "rain_dry_type", col, RAINY_LABEL, DRY_LABEL)
        res["feature"] = feature
        res["feature_col"] = col
        t_rows.append(res)
    t_df = pd.DataFrame(t_rows)
    if not t_df.empty:
        ...
        t_df.to_csv(STATS_DIR / "stats_05_audiofeatures_rain_vs_dry_tests.csv", index=False)
        t_df.to_csv(POWERBI_DIR / "pbi_05_audiofeatures_rain_vs_dry_tests.csv", index=False)
        ...
        for _, row in t_df.iterrows():
            add_significance_row(
                ...
                variable=row["feature"],
                comparison=f"{RAINY_LABEL} vs {DRY_LABEL}",
                p_value=row["welch_p"],
    ...
    if focus_cols:
        ...
        for ax, (feature, col) in zip(axes, focus_cols.items()):
            subset = daily[daily["rain_dry_type"].isin([RAINY_LABEL, DRY_LABEL])]
            rainy = subset.loc[subset["rain_dry_type"] == RAINY_LABEL, col].dropna()
            dry = subset.loc[subset["rain_dry_type"] == DRY_LABEL, col].dropna()
        ...
        for j in range(n_metrics, len(axes)):
            fig.delaxes(axes[j])
        fig.suptitle("Audiofeatures op regendagen vs droge dagen", fontsize=14, fontweight="bold")
        fig.tight_layout(rect=[0, 0, 1, 0.95])
        fig.savefig(FIG_DIR / "fig_06_audiofeatures_rain_vs_dry_boxplots.png", dpi=200)
        plt.close(fig)
...
def analyse_rain_dry_boxplots(daily, significance_rows):
    analysis_name = "08_rain_vs_dry_boxplots"
    ...
    for label, col in metric_map.items():
        for _, row in daily[["date", "rain_dry_type", col, "prcp", "tsun"]].dropna(subset=[col]).iterrows():
            long_rows.append({
                "date": row["date"].strftime("%Y-%m-%d"),
                "rain_dry_type": row["rain_dry_type"],
                "metric_label": label,
        ...
        add_significance_row(significance_rows, analysis_name, "Welch t-test", label, f"{RAINY_LABEL} vs {DRY_LABEL}", res["welch_p"], res["welch_t"], res["n_a"] + res["n_b"])

    boxplot_data = pd.DataFrame(long_rows)
    boxplot_data.to_csv(POWERBI_DIR / "pbi_07_rain_vs_dry_boxplot_data.csv", index=False)
    boxplot_data.to_csv(TABLE_DIR / "table_07_rain_vs_dry_boxplot_data.csv", index=False)

    stat_df = pd.DataFrame(stat_rows)
    stat_df.to_csv(STATS_DIR / "stats_07_rain_vs_dry_boxplot_tests.csv", index=False)
    stat_df.to_csv(POWERBI_DIR / "pbi_07_rain_vs_dry_boxplot_tests.csv", index=False)
    ...
    fig.suptitle("Vergelijking tussen regendagen en droge dagen", fontsize=14, fontweight="bold")
    fig.tight_layout(rect=[0, 0, 1, 0.95])
    fig.savefig(FIG_DIR / "fig_07_rain_vs_dry_boxplots.png", dpi=200)
    plt.close(fig)
...
def write_paper_summary(significance_df):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("Hoofdwijzigingen in v3:\n")
        f.write("- Hoofdmetric bevat nu ook gemiddelde valence van de Top 200 op regendagen vs droge dagen.\n")
        f.write("- Geen neutrale weercategorie in de finale vergelijkingen.\n")
...
def main():
    ...
    analyse_rain_dry_boxplots(daily, significance_rows)
    ...
    print("- pbi_01_main_rain_vs_dry_tests.csv")
    ...
    print("- pbi_07_rain_vs_dry_boxplot_data.csv")
