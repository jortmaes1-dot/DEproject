...
# ============================================================
# VISUELE INSTELLINGEN VOOR DUIDELIJKE, MODERNE FIGUREN
# ============================================================
# De analyses blijven exact hetzelfde. Deze helperfuncties zorgen alleen voor:
# - meer kleur
# - betere y-as-schalen
# - duidelijke labels
# - minder rommel in de grafieken
# - hogere resolutie voor rapporten en presentaties
# ============================================================

COLOR_BAD = "#E76F51"
COLOR_GOOD = "#2A9D8F"
COLOR_NEUTRAL = "#8D99AE"
COLOR_DARK = "#264653"
COLOR_ACCENT = "#F4A261"
COLOR_BLUE = "#457B9D"
COLOR_GRID = "#D9DEE8"
...
def style_axis(ax, grid_axis="y"):
    ...
    ax.spines["left"].set_color("#C7CEDB")
    ax.spines["bottom"].set_color("#C7CEDB")
    if grid_axis:
        ax.grid(axis=grid_axis, alpha=0.35, color=COLOR_GRID, linewidth=0.8)
    ax.set_axisbelow(True)


def is_share_metric(metric_name):
    metric_name = str(metric_name).lower()
    return any(word in metric_name for word in ["share", "aandeel", "percentage", "weighted_share"])


def set_smart_ylim(ax, values, share=False, zero_baseline=False):
    ...
    if np.isclose(vmin, vmax):
        margin = 0.03 if share else max(abs(vmin) * 0.08, 0.05)
    else:
        margin = (vmax - vmin) * 0.35
    ...
    if share:
        ...
        if upper - lower < 0.06:
            ...
            lower = max(0, center - 0.03)
            upper = min(1, center + 0.03)
...
def add_bar_labels(ax, values, share=False, fontsize=9):
    values = list(values)
    ...
    for patch, value in zip(ax.patches, values):
        if pd.isna(value):
            continue
        label = f"{value:.1%}" if share else f"{value:.3f}"
        ax.text(
...
def save_fig(fig, filename, dpi=260):
    fig.tight_layout()
    fig.savefig(FIG_DIR / filename, dpi=dpi)
    plt.close(fig)


def styled_boxplot(ax, data, labels, colors, ylabel=None, title=None, share=False):
    clean_data = [pd.to_numeric(pd.Series(x), errors="coerce").dropna() for x in data]
    box = ax.boxplot(
        clean_data,
        tick_labels=labels,
        patch_artist=True,
        widths=0.55,
        showmeans=True,
        meanprops={"marker": "D", "markerfacecolor": "white", "markeredgecolor": COLOR_DARK, "markersize": 5},
        medianprops={"color": COLOR_DARK, "linewidth": 2},
        boxprops={"linewidth": 1.2, "color": "#455A64"},
        whiskerprops={"linewidth": 1.1, "color": "#455A64"},
        capprops={"linewidth": 1.1, "color": "#455A64"},
        flierprops={"marker": "o", "markersize": 3, "markerfacecolor": "#90A4AE", "markeredgecolor": "#90A4AE", "alpha": 0.45},
    )
    ...
    style_axis(ax)
    if clean_data:
        all_values = pd.concat(clean_data, ignore_index=True)
        set_smart_ylim(ax, all_values, share=share)


def plot_weather_metric_overview(daily, metric_col, title, filename, ylabel):
    if metric_col not in daily.columns or daily[metric_col].notna().sum() == 0:
        return

    d = daily.sort_values("date").dropna(subset=[metric_col]).copy()
    if d.empty:
        return

    d["rolling_14"] = d[metric_col].rolling(14, min_periods=4).mean()
    share = is_share_metric(metric_col) or is_share_metric(title)

    fig, ax = plt.subplots(figsize=(13, 5.8))

    bad_days = d[d["good_bad_weather_type"] == BAD_WEATHER_LABEL]
    good_days = d[d["good_bad_weather_type"] == GOOD_WEATHER_LABEL]

    for _, row in bad_days.iterrows():
        ax.axvspan(row["date"], row["date"] + pd.Timedelta(days=1), color=COLOR_BAD, alpha=0.08, linewidth=0)
    for _, row in good_days.iterrows():
        ax.axvspan(row["date"], row["date"] + pd.Timedelta(days=1), color=COLOR_GOOD, alpha=0.08, linewidth=0)

    ax.plot(d["date"], d[metric_col], color=COLOR_NEUTRAL, linewidth=1.1, alpha=0.45, label="Dagwaarde")
    ax.plot(d["date"], d["rolling_14"], color=COLOR_DARK, linewidth=2.8, label="14-daags gemiddelde")

    ax.set_title(title)
    ax.set_xlabel("Datum")
    ax.set_ylabel(ylabel)
    set_smart_ylim(ax, d[metric_col], share=share)
    style_axis(ax)
    ax.legend(loc="best")
...
def plot_weather_bars(plot_df, metric, metric_label, ax):
    order_map = {BAD_WEATHER_LABEL: 1, GOOD_WEATHER_LABEL: 2}
    plot_df = plot_df.copy()
    plot_df["order"] = plot_df["good_bad_weather_type"].map(order_map)
    plot_df = plot_df.sort_values("order")

    colors = [COLOR_BAD if x == BAD_WEATHER_LABEL else COLOR_GOOD for x in plot_df["good_bad_weather_type"]]
    xlabels = [
        f"Regenachtig / slecht\n(n={int(n)})" if w == BAD_WEATHER_LABEL else f"Zonnig / goed\n(n={int(n)})"
        for w, n in zip(plot_df["good_bad_weather_type"], plot_df["n_days"])
    ]
    values = plot_df["mean_value"].astype(float).values
    errors = plot_df["sd_value"].astype(float).values if "sd_value" in plot_df.columns else None

    ax.bar(xlabels, values, color=colors, alpha=0.9, edgecolor="white", linewidth=1.4)
    if errors is not None and not np.isnan(errors).all():
        ax.errorbar(range(len(values)), values, yerr=errors, fmt="none", ecolor="#37474F", elinewidth=1.1, capsize=4, alpha=0.75)

    share = is_share_metric(metric) or is_share_metric(metric_label)
    set_smart_ylim(ax, values, share=share)
    add_bar_labels(ax, values, share=share)
...
def horizontal_correlation_plot(df, label_col, corr_col, title, xlabel, filename, weather_var, top_n_each_side=6):
    ...
    colors = [COLOR_GOOD if value >= 0 else COLOR_BAD for value in combined[corr_col]]
    fig_height = max(5.5, 0.42 * len(combined) + 1.5)
    fig, ax = plt.subplots(figsize=(11, fig_height))
    ax.barh(combined[label_col].astype(str), combined[corr_col], color=colors, alpha=0.88)
    ax.axvline(0, linewidth=1.2, color="#263238")

    max_abs = max(0.05, float(combined[corr_col].abs().max()))
    ax.set_xlim(-max_abs * 1.18, max_abs * 1.18)

    for y, value in enumerate(combined[corr_col]):
        ha = "left" if value >= 0 else "right"
        offset = max_abs * 0.035 if value >= 0 else -max_abs * 0.035
        ax.text(value + offset, y, f"{value:.2f}", va="center", ha=ha, fontsize=9, fontweight="bold", color="#263238")
...
def analyse_main_weather(daily, significance_rows):
    ...
    # Figuur: duidelijke, gekleurde barplot zonder scatter.
    ...
    for ax, (metric, metric_label) in zip(axes, metric_info.items()):
        ...
        plot_weather_bars(plot_df, metric, metric_label, ax)
    fig.suptitle("Hoofdonderzoek: regenachtig/slecht weer vs zonnig/goed weer", fontsize=15, fontweight="bold", color=COLOR_DARK)
    fig.tight_layout(rect=[0, 0, 1, 0.92])
    fig.savefig(FIG_DIR / "fig_01_main_good_vs_bad_weather_bars.png", dpi=260)
    plt.close(fig)

    # Extra overzichtsfiguren: tijdslijn met 14-daags gemiddelde en weermarkering.
    plot_weather_metric_overview(
        daily,
        "main_avg_valence_metric",
        "Gemiddelde valence van de Top 200 doorheen de tijd",
        "fig_01b_timeline_avg_valence_weather.png",
        "Gemiddelde valence",
    )
    plot_weather_metric_overview(
        daily,
        "main_sad_share_metric",
        "Aandeel sad songs/sad streams doorheen de tijd",
        "fig_01c_timeline_sad_share_weather.png",
        "Aandeel sad songs/sad streams",
    )
...
def analyse_seasons(daily, significance_rows):
    ...
    # Grafieken: kleurrijke seizoenbars met slimme y-as en waardelabels.
    fig, ax = plt.subplots(figsize=(10.5, 5.7))
    colors = [SEASON_COLORS.get(s, COLOR_NEUTRAL) for s in season_summary["season"]]
    values = season_summary["mean_sad_share"].astype(float).values
    labels = [f"{s}\n(n={int(n)})" for s, n in zip(season_summary["season"], season_summary["n_days"])]
    ax.bar(labels, values, color=colors, alpha=0.92, edgecolor="white", linewidth=1.4)
    ax.set_title("Gemiddeld aandeel sad songs per seizoen")
    ax.set_xlabel("Seizoen")
    ax.set_ylabel("Aandeel sad songs/sad streams")
    set_smart_ylim(ax, values, share=True)
    add_bar_labels(ax, values, share=True)
    style_axis(ax)
    save_fig(fig, "fig_02a_season_sad_share.png")

    fig, ax = plt.subplots(figsize=(10.5, 5.7))
    values = season_summary["mean_avg_valence"].astype(float).values
    ax.bar(labels, values, color=colors, alpha=0.92, edgecolor="white", linewidth=1.4)
    ax.set_title("Gemiddelde valence van de Top 200 per seizoen")
    ax.set_xlabel("Seizoen")
    ax.set_ylabel("Gemiddelde valence")
    set_smart_ylim(ax, values, share=False)
    add_bar_labels(ax, values, share=False)
    style_axis(ax)
    save_fig(fig, "fig_02b_season_avg_valence.png")

    # Extra gecombineerde figuur: valence en sad share naast elkaar voor snelle rapportering.
    fig, axes = plt.subplots(1, 2, figsize=(14.5, 5.8))
    for ax, metric, title, share in [
        (axes[0], "mean_avg_valence", "Gemiddelde valence", False),
        (axes[1], "mean_sad_share", "Aandeel sad songs/sad streams", True),
    ]:
        values = season_summary[metric].astype(float).values
        ax.bar(labels, values, color=colors, alpha=0.92, edgecolor="white", linewidth=1.4)
        ax.set_title(title)
        set_smart_ylim(ax, values, share=share)
        add_bar_labels(ax, values, share=share)
        style_axis(ax)
    fig.suptitle("Seizoensvergelijking Spotify Top 200", fontsize=15, fontweight="bold", color=COLOR_DARK)
    fig.tight_layout(rect=[0, 0, 1, 0.92])
    fig.savefig(FIG_DIR / "fig_02c_season_combined_overview.png", dpi=260)
    plt.close(fig)
...
def plot_signed_correlations(df, label_col, corr_col, title, xlabel, filename, weather_var, top_n_each_side=6):
    horizontal_correlation_plot(df, label_col, corr_col, title, xlabel, filename, weather_var, top_n_each_side=top_n_each_side)
...
def analyse_audiofeatures_weather(daily, significance_rows):
    ...
    if not strength_summary.empty:
        ...
        colors = [FEATURE_COLORS.get(feature, COLOR_BLUE) for feature in strength_summary["feature"]]
        ax.barh(strength_summary["feature"], strength_summary["max_abs_corr_weather"], color=colors, alpha=0.9)
        ax.set_title("Sterkte van verband per audiofeature met weer")
        ax.set_xlabel("Grootste absolute correlatie met prcp of tsun")
        ax.set_ylabel("Spotify-audiofeature")
        ax.set_xlim(0, max(0.05, float(strength_summary["max_abs_corr_weather"].max()) * 1.18))
        for y, value in enumerate(strength_summary["max_abs_corr_weather"]):
            ax.text(value + ax.get_xlim()[1] * 0.012, y, f"{value:.2f}", va="center", fontsize=9, fontweight="bold")
        style_axis(ax, grid_axis="x")
        save_fig(fig, "fig_05_audiofeatures_weather_strength.png")
    ...
    if focus_cols:
        ...
        for ax, (feature, col) in zip(axes, focus_cols.items()):
            ...
            styled_boxplot(
                ax,
                [rainy, dry],
                labels=[f"Regenachtig\n(n={len(rainy)})", f"Goed weer\n(n={len(dry)})"],
                colors=[COLOR_BAD, COLOR_GOOD],
                ylabel="Dagelijks gewogen gemiddelde",
...
def analyse_good_bad_weather_boxplots(daily, significance_rows):
    ...
    for ax, (label, col) in zip(axes, metric_map.items()):
        ...
        styled_boxplot(
            ...
            ylabel=None,
            title=label,
            share=is_share_metric(label) or is_share_metric(col),
        )
